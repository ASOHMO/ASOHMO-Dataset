...
averages = {}
for filename in glob("./test_results_ijcai/test/single_prediction/*"):
    filename_splitted = filename.split("/")[-1].split("_")
    print(filename_splitted)
    is_type_of_premise = len(filename_splitted) == 8
    if filename_splitted[1] == "test" and filename_splitted[0] == "results":
        ...
        if is_type_of_premise:
            key = "{}_{}_{}_{}_Type-of-premise".format(component, modelname, lr, batch_size)
        for line in f:
